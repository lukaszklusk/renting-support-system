...
public class MessagesController {
    ...
    public ResponseEntity<Message> postMessageAsResponse(@PathVariable long id,
        ...
        try {
            Optional<Message> replyOptional = service.getMessageById(id);
            ...
            LocalDateTime sendTime = LocalDateTime.parse((String) payload.get("sendTime"),
                    DateTimeFormatter.ISO_LOCAL_DATE_TIME);
            String content = (String) payload.get("content");
            Message message = service.createMessageWithResponse(senderOptional.get(), receiverOptional.get(), content, sendTime, replyOptional.get());
            return ResponseEntity.ok(message);
        }catch (DateTimeParseException | IllegalArgumentException e){
            return ResponseEntity.badRequest().build();
        }
